object frmProdutos: TfrmProdutos
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'frmProdutos'
  ClientHeight = 930
  ClientWidth = 1340
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -19
  Font.Name = 'Segoe UI'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Touch.GestureManager = GestureManager1
  Touch.InteractiveGestureOptions = [igoPanSingleFingerHorizontal, igoPanInertia, igoParentPassthrough]
  WindowState = wsMaximized
  OnGesture = FormGesture
  OnKeyDown = FormKeyDown
  OnResize = FormResize
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 25
  object AppBar: TPanel
    Left = 0
    Top = 855
    Width = 1340
    Height = 75
    BevelOuter = bvNone
    TabOrder = 0
    Visible = False
    object CloseButton: TImage
      Left = 1280
      Top = 0
      Width = 60
      Height = 75
      Align = alRight
      Center = True
      Picture.Data = {
        
        7F018E704185241CEA460000000049454E44AE426082}
      OnClick = CloseButtonClick
      ExplicitLeft = 1936
    end
  end
  object cxpcProdutos: TcxPageControl
    Left = 0
    Top = 367
    Width = 1340
    Height = 563
    Align = alClient
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Segoe UI'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    Properties.ActivePage = cxtsPrincipal
    Properties.CustomButtons.Buttons = <>
    LookAndFeel.NativeStyle = False
    ClientRectBottom = 563
    ClientRectRight = 1340
    ClientRectTop = 24
    object cxtsPrincipal: TcxTabSheet
      Caption = 'Produto'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Segoe UI'
      Font.Style = []
      ImageIndex = 0
      ParentFont = False
      object Image1: TImage
        Left = 608
        Top = 8
        Width = 220
        Height = 241
        Stretch = True
      end
      object DBEdit1: TDBEdit
        Left = 3
        Top = 24
        Width = 66
        Height = 21
        DataField = 'PRO_ID'
        DataSource = DM.dsProdutos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
        OnChange = DBEdit1Change
      end
      object dbeDescricaoProduto: TDBEdit
        Left = 88
        Top = 24
        Width = 241
        Height = 21
        DataField = 'PRO_DESCRICAO'
        DataSource = DM.dsProdutos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
      end
      object DBEdit3: TDBEdit
        Left = 335
        Top = 24
        Width = 90
        Height = 21
        DataField = 'PRO_CODIGO'
        DataSource = DM.dsProdutos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 2
      end
      object DBEdit2: TDBEdit
        Left = 312
        Top = 75
        Width = 113
        Height = 21
        DataField = 'CODIGO_NCM'
        DataSource = DM.dsProdutos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 6
      end
      object DBEdit6: TDBEdit
        Left = 215
        Top = 75
        Width = 80
        Height = 21
        DataField = 'PRECO_VENDA'
        DataSource = DM.dsProdutos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 5
      end
      object DBEdit5: TDBEdit
        Left = 135
        Top = 75
        Width = 74
        Height = 21
        DataField = 'PRO_SALDO'
        DataSource = DM.dsProdutos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        ReadOnly = True
        TabOrder = 4
      end
      object DBEEan13: TDBEdit
        Left = 3
        Top = 75
        Width = 126
        Height = 21
        DataField = 'PRO_EAN13'
        DataSource = DM.dsProdutos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 3
      end
      object DBEDescricaoGrupo: TDBEdit
        Left = 3
        Top = 125
        Width = 239
        Height = 21
        DataField = 'GRUPO_PRODUTO_DESCRICAO'
        DataSource = DM.dsGruposProdutos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 8
      end
      object cxBtnCapturarGrupo: TcxButton
        Left = 246
        Top = 122
        Width = 97
        Height = 25
        Caption = 'Capturar Grupo'
        Enabled = False
        TabOrder = 9
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        OnClick = cxBtnCapturarGrupoClick
      end
      object DBGridGrupoProdutos: TDBGrid
        Left = 3
        Top = 160
        Width = 340
        Height = 128
        DataSource = DM.dsGruposProdutos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 10
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -19
        TitleFont.Name = 'Segoe UI'
        TitleFont.Style = []
        Visible = False
        OnDblClick = DBGridGrupoProdutosDblClick
        Columns = <
          item
            Expanded = False
            FieldName = 'GRUPO_PRODUTO_DESCRICAO'
            Title.Caption = 'Grupo Produtos'
            Visible = True
          end>
      end
      object cxButton3: TcxButton
        Left = 608
        Top = 255
        Width = 107
        Height = 33
        Caption = 'Copiar Imagem'
        LookAndFeel.NativeStyle = False
        TabOrder = 11
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        OnClick = cxButton3Click
      end
      object cxButton4: TcxButton
        Left = 716
        Top = 255
        Width = 112
        Height = 33
        Caption = 'Apagar Imagem'
        LookAndFeel.NativeStyle = False
        TabOrder = 12
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        OnClick = cxButton4Click
      end
      object cxLabel1: TcxLabel
        Left = 3
        Top = 8
        Caption = 'Identificador'
        Style.LookAndFeel.NativeStyle = False
        StyleDisabled.LookAndFeel.NativeStyle = False
        StyleFocused.LookAndFeel.NativeStyle = False
        StyleHot.LookAndFeel.NativeStyle = False
        Transparent = True
      end
      object cxLabel2: TcxLabel
        Left = 88
        Top = 8
        Caption = 'Descri'#231#227'o do Produto'
        Style.LookAndFeel.NativeStyle = False
        StyleDisabled.LookAndFeel.NativeStyle = False
        StyleFocused.LookAndFeel.NativeStyle = False
        StyleHot.LookAndFeel.NativeStyle = False
        Transparent = True
      end
      object cxLabel3: TcxLabel
        Left = 335
        Top = 8
        Caption = 'C'#243'digo Interno'
        Style.LookAndFeel.NativeStyle = False
        StyleDisabled.LookAndFeel.NativeStyle = False
        StyleFocused.LookAndFeel.NativeStyle = False
        StyleHot.LookAndFeel.NativeStyle = False
        Transparent = True
      end
      object cxLabel4: TcxLabel
        Left = 3
        Top = 59
        Caption = 'EAN 13'
        Style.LookAndFeel.NativeStyle = False
        StyleDisabled.LookAndFeel.NativeStyle = False
        StyleFocused.LookAndFeel.NativeStyle = False
        StyleHot.LookAndFeel.NativeStyle = False
        Transparent = True
      end
      object cxLabel5: TcxLabel
        Left = 135
        Top = 59
        Caption = 'Saldo'
        Style.LookAndFeel.NativeStyle = False
        StyleDisabled.LookAndFeel.NativeStyle = False
        StyleFocused.LookAndFeel.NativeStyle = False
        StyleHot.LookAndFeel.NativeStyle = False
        Transparent = True
      end
      object cxLabel6: TcxLabel
        Left = 215
        Top = 59
        Caption = 'Pre'#231'o de Venda'
        Style.LookAndFeel.NativeStyle = False
        StyleDisabled.LookAndFeel.NativeStyle = False
        StyleFocused.LookAndFeel.NativeStyle = False
        StyleHot.LookAndFeel.NativeStyle = False
        Transparent = True
      end
      object cxLabel7: TcxLabel
        Left = 312
        Top = 59
        Caption = 'C'#243'digo NCM'
        Style.LookAndFeel.NativeStyle = False
        StyleDisabled.LookAndFeel.NativeStyle = False
        StyleFocused.LookAndFeel.NativeStyle = False
        StyleHot.LookAndFeel.NativeStyle = False
        Transparent = True
      end
      object cxLabel8: TcxLabel
        Left = 3
        Top = 107
        Caption = 'Grupo Produto'
        Style.LookAndFeel.NativeStyle = False
        StyleDisabled.LookAndFeel.NativeStyle = False
        StyleFocused.LookAndFeel.NativeStyle = False
        StyleHot.LookAndFeel.NativeStyle = False
        Transparent = True
      end
      object DBEdit8: TDBEdit
        Left = 352
        Top = 125
        Width = 21
        Height = 21
        DataField = 'ORIGEM_MERCADORIA'
        DataSource = DM.dsProdutos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 7
        OnChange = DBEdit8Change
      end
      object cxLabel11: TcxLabel
        Left = 352
        Top = 107
        Caption = 'origem mercadoria'
        Style.LookAndFeel.NativeStyle = False
        StyleDisabled.LookAndFeel.NativeStyle = False
        StyleFocused.LookAndFeel.NativeStyle = False
        StyleHot.LookAndFeel.NativeStyle = False
        Transparent = True
      end
      object lblOrigemMercadoria: TcxLabel
        Left = 377
        Top = 126
        AutoSize = False
        Caption = 'origem mercadoria'
        Style.LookAndFeel.NativeStyle = False
        StyleDisabled.LookAndFeel.NativeStyle = False
        StyleFocused.LookAndFeel.NativeStyle = False
        StyleHot.LookAndFeel.NativeStyle = False
        Properties.WordWrap = True
        Transparent = True
        Height = 35
        Width = 225
      end
      object cxLabel9: TcxLabel
        Left = 352
        Top = 160
        Caption = 'CFOP'
        Style.LookAndFeel.NativeStyle = False
        StyleDisabled.LookAndFeel.NativeStyle = False
        StyleFocused.LookAndFeel.NativeStyle = False
        StyleHot.LookAndFeel.NativeStyle = False
        Transparent = True
      end
      object dbedtCODIGO_NCM: TDBEdit
        Left = 352
        Top = 176
        Width = 97
        Height = 21
        DataField = 'CFOP'
        DataSource = DM.dsProdutos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 24
      end
      object cxLabel10: TcxLabel
        Left = 352
        Top = 208
        Caption = 'Unidade'
        Style.LookAndFeel.NativeStyle = False
        StyleDisabled.LookAndFeel.NativeStyle = False
        StyleFocused.LookAndFeel.NativeStyle = False
        StyleHot.LookAndFeel.NativeStyle = False
        Transparent = True
      end
      object dbedtPRO_EAN13: TDBEdit
        Left = 352
        Top = 224
        Width = 99
        Height = 21
        DataField = 'UNIDADE'
        DataSource = DM.dsProdutos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 26
      end
      object cxLabel12: TcxLabel
        Left = 436
        Top = 8
        Caption = 'Cor'
        Style.LookAndFeel.NativeStyle = False
        StyleDisabled.LookAndFeel.NativeStyle = False
        StyleFocused.LookAndFeel.NativeStyle = False
        StyleHot.LookAndFeel.NativeStyle = False
        Transparent = True
      end
      object DBEdit4: TDBEdit
        Left = 439
        Top = 24
        Width = 90
        Height = 21
        DataField = 'COR'
        DataSource = DM.dsProdutos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 28
      end
      object DBEdit9: TDBEdit
        Left = 439
        Top = 75
        Width = 90
        Height = 21
        DataField = 'TAMANHO'
        DataSource = DM.dsProdutos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 29
      end
      object cxLabel14: TcxLabel
        Left = 439
        Top = 59
        Caption = 'Tamanho'
        Style.LookAndFeel.NativeStyle = False
        StyleDisabled.LookAndFeel.NativeStyle = False
        StyleFocused.LookAndFeel.NativeStyle = False
        StyleHot.LookAndFeel.NativeStyle = False
        Transparent = True
      end
      object cxLabel13: TcxLabel
        Left = 466
        Top = 160
        Caption = 'Estoque minimo'
        Style.LookAndFeel.NativeStyle = False
        StyleDisabled.LookAndFeel.NativeStyle = False
        StyleFocused.LookAndFeel.NativeStyle = False
        StyleHot.LookAndFeel.NativeStyle = False
        Transparent = True
      end
      object DBEdit7: TDBEdit
        Left = 469
        Top = 176
        Width = 90
        Height = 21
        DataField = 'MINIMO'
        DataSource = DM.dsProdutos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 32
      end
      object cxLabel15: TcxLabel
        Left = 469
        Top = 211
        Caption = 'Estoque m'#225'ximo'
        Style.LookAndFeel.NativeStyle = False
        StyleDisabled.LookAndFeel.NativeStyle = False
        StyleFocused.LookAndFeel.NativeStyle = False
        StyleHot.LookAndFeel.NativeStyle = False
        Transparent = True
      end
      object DBEdit10: TDBEdit
        Left = 469
        Top = 227
        Width = 90
        Height = 21
        DataField = 'MAXIMO'
        DataSource = DM.dsProdutos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 34
      end
      object cxLabel16: TcxLabel
        Left = 350
        Top = 249
        Caption = 'Pre'#231'o Custo'
        Style.LookAndFeel.NativeStyle = False
        StyleDisabled.LookAndFeel.NativeStyle = False
        StyleFocused.LookAndFeel.NativeStyle = False
        StyleHot.LookAndFeel.NativeStyle = False
        Transparent = True
      end
      object DBEdit11: TDBEdit
        Left = 351
        Top = 265
        Width = 101
        Height = 21
        DataField = 'CUSTO'
        DataSource = DM.dsProdutos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 36
      end
    end
    object cxtsMovimentacoes: TcxTabSheet
      Caption = 'Movimenta'#231#245'es'
      ImageIndex = 1
      object cxGridMovimentacoes: TcxGrid
        Left = 0
        Top = 0
        Width = 1340
        Height = 539
        Align = alClient
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        LookAndFeel.NativeStyle = False
        object cxGridMovimentacoesDBTableView1: TcxGridDBTableView
          Navigator.Buttons.CustomButtons = <>
          DataController.DataSource = DM.dsMovimentacoes
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <>
          DataController.Summary.SummaryGroups = <>
          object cxGridMovimentacoesDBTableView1ID_MOVIMENTACAO: TcxGridDBColumn
            Caption = 'Identificador'
            DataBinding.FieldName = 'ID_MOVIMENTACAO'
            Width = 70
          end
          object cxGridMovimentacoesDBTableView1ID_PEDIDO: TcxGridDBColumn
            Caption = 'N'#186' Pedido'
            DataBinding.FieldName = 'ID_PEDIDO'
          end
          object cxGridMovimentacoesDBTableView1ID_PRODUTO: TcxGridDBColumn
            Caption = 'Identif. Produto'
            DataBinding.FieldName = 'ID_PRODUTO'
            Width = 90
          end
          object cxGridMovimentacoesDBTableView1DATA: TcxGridDBColumn
            Caption = 'Data da venda'
            DataBinding.FieldName = 'DATA'
            Width = 80
          end
          object cxGridMovimentacoesDBTableView1HORA: TcxGridDBColumn
            Caption = 'Hora da venda'
            DataBinding.FieldName = 'HORA'
            Width = 80
          end
          object cxGridMovimentacoesDBTableView1QTDE_ANTERIOR: TcxGridDBColumn
            Caption = 'Qtde. Anterior'
            DataBinding.FieldName = 'QTDE_ANTERIOR'
            Width = 90
          end
          object cxGridMovimentacoesDBTableView1QTDE_MOVIMENTADA: TcxGridDBColumn
            Caption = 'Qtde. Movimentada'
            DataBinding.FieldName = 'QTDE_MOVIMENTADA'
          end
          object cxGridMovimentacoesDBTableView1QTDE_APOS_MOVIMENTACAO: TcxGridDBColumn
            Caption = 'Saldo Ap'#243's Mov.'
            DataBinding.FieldName = 'QTDE_APOS_MOVIMENTACAO'
            Width = 90
          end
          object cxGridMovimentacoesDBTableView1TIPO_MOVIMENTACAO: TcxGridDBColumn
            Caption = 'Tipo Movimenta'#231#227'o'
            DataBinding.FieldName = 'TIPO_MOVIMENTACAO'
            Width = 200
          end
        end
        object cxGridMovimentacoesLevel1: TcxGridLevel
          GridView = cxGridMovimentacoesDBTableView1
        end
      end
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 326
    Width = 1340
    Height = 41
    Align = alTop
    TabOrder = 2
    object Gauge1: TGauge
      Left = 703
      Top = 10
      Width = 193
      Height = 25
      Progress = 0
      Visible = False
    end
    object cxNavigator1: TcxNavigator
      Left = 11
      Top = 10
      Width = 227
      Height = 25
      Control = cxGrid1DBTableView1
      Buttons.OnButtonClick = cxNavigator1ButtonsButtonClick
      Buttons.CustomButtons = <>
      Buttons.First.Hint = 'Primeiro registro'
      Buttons.PriorPage.Visible = False
      Buttons.Prior.Hint = 'Pr'#243'ximo registro'
      Buttons.Next.Hint = 'Pr'#243'ximo registro'
      Buttons.NextPage.Visible = False
      Buttons.Last.Hint = #218'ltimo registro'
      Buttons.Insert.Hint = 'Inserir'
      Buttons.Delete.Hint = 'Excluir'
      Buttons.Edit.Hint = 'Editar'
      Buttons.Post.Hint = 'Salvar'
      Buttons.Cancel.Hint = 'Cancelar'
      Buttons.Refresh.Visible = False
      Buttons.SaveBookmark.Visible = False
      Buttons.GotoBookmark.Visible = False
      Buttons.Filter.Visible = False
      LookAndFeel.NativeStyle = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
    end
    object cxBtnEntrada: TcxButton
      Left = 245
      Top = 10
      Width = 75
      Height = 25
      Caption = 'Entrada'
      LookAndFeel.NativeStyle = False
      TabOrder = 1
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
      OnClick = cxBtnEntradaClick
    end
    object cxBtnSaida: TcxButton
      Left = 325
      Top = 10
      Width = 75
      Height = 25
      Caption = 'Sa'#237'da'
      LookAndFeel.NativeStyle = False
      TabOrder = 2
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
      OnClick = cxBtnSaidaClick
    end
    object cxBtnEntradaSobFiltro: TcxButton
      Left = 406
      Top = 10
      Width = 179
      Height = 25
      Caption = 'Entrada sob filtro'
      LookAndFeel.NativeStyle = False
      TabOrder = 3
      OnClick = cxBtnEntradaSobFiltroClick
    end
    object BtImportar: TcxButton
      Left = 591
      Top = 10
      Width = 100
      Height = 25
      Caption = 'Importar'
      TabOrder = 4
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -19
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
      OnClick = BtImportarClick
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 54
    Width = 1340
    Height = 272
    Align = alTop
    TabOrder = 3
    object cxGrid1: TcxGrid
      Left = 1
      Top = 1
      Width = 1338
      Height = 270
      Align = alClient
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      LookAndFeel.NativeStyle = False
      object cxGrid1DBTableView1: TcxGridDBTableView
        Navigator.Buttons.CustomButtons = <>
        DataController.DataSource = DM.dsProdutos
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <>
        DataController.Summary.SummaryGroups = <>
        object cxGrid1DBTableView1PRO_ID: TcxGridDBColumn
          Caption = 'Identificador'
          DataBinding.FieldName = 'PRO_ID'
          Width = 100
        end
        object cxGrid1DBTableView1PRO_CODIGO: TcxGridDBColumn
          Caption = 'C'#243'digo interno'
          DataBinding.FieldName = 'PRO_CODIGO'
          Width = 105
        end
        object cxGrid1DBTableView1PRO_DESCRICAO: TcxGridDBColumn
          Caption = 'Descri'#231#227'o'
          DataBinding.FieldName = 'PRO_DESCRICAO'
          Width = 300
        end
      end
      object cxGrid1Level1: TcxGridLevel
        GridView = cxGrid1DBTableView1
      end
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 1340
    Height = 54
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 4
    object Label1: TLabel
      Left = 96
      Top = -11
      Width = 209
      Height = 74
      Caption = 'Produtos'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -56
      Font.Name = 'Segoe UI Light'
      Font.Style = []
      ParentFont = False
    end
    object Image2: TImage
      Left = 12
      Top = 3
      Width = 53
      Height = 47
      Picture.Data = {
        
        49454E44AE426082}
      OnClick = BackToMainForm
    end
  end
  object ActionList1: TActionList
    Left = 120
    Top = 32
    object Action1: TAction
      Caption = 'Action1'
      OnExecute = Action1Execute
    end
  end
  object GestureManager1: TGestureManager
    Left = 48
    Top = 32
    GestureData = <
      item
        Control = Owner
        Collection = <
          item
            Action = Action1
            GestureID = sgiUp
          end>
      end>
  end
end
